...
abstract class WheelExtension(val project: Project) {
    ...
    var publish = PublishingConfig()
    var run = RunDirectory()

    fun publish(action: Closure<Void?>?) {
    ...
    fun java(action: Closure<Void?>?) {
        project.configure(this.java, action)
    }
    ...
    companion object {
        val repositories: RepositoryContainer = RepositoryContainer().configure {dependencies ->
            dependencies.repository("auoeke", "https://maven.auoeke.net")
        ...
        fun repository(key: String, value: String) {
            repositories.putRepository(key, value)
        }

        fun dependency(key: String): Dependency {
